...
function sourceCard(citation, index, evidence, citedClaims) {
  ...
  return `<article class="source-card" id="source-${escapeHtml(citation.chunk_id)}" tabindex="-1"><header><span class="index">${index + 1}</span><div><p class="passage-label">${index === 0 ? 'Primary evidence' : 'Supporting evidence'}</p><h3>${escapeHtml(citation.document_title)}</h3>${page ? `<small>${escapeHtml(page)}</small>` : ''}</div></header><p class="passage-label">Relevant passage</p><blockquote>${escapeHtml(excerpt)}</blockquote>${citation.source_url ? `<a href="${escapeHtml(citation.source_url)}" target="_blank" rel="noreferrer">View original RBI source →</a>` : ''}</article>`;
}
...
async function loadStatus() {
  try {
    ...
    $('#knowledge-summary').textContent = relativeTime(latest); $('#header-last-check').textContent = relativeTime(latest); $('#knowledge-health').textContent = status.health === 'healthy' ? '● Live' : 'Needs review'; $('#source-live').classList.toggle('healthy', status.health === 'healthy');
  } catch {
